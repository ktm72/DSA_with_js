...
class Printer extends Queue {
  ...
  constructor() {
    super(10); //super class constructor
  ...
  #printDocs = () => {
    ...
    }, Math.floor(Math.random() * 2000)); //doc printing time
  };
  ...
  print(doc) {
    return new Promise((res, rej) => {
      if (this.isFull) {
        ...
        this.enqueue(() => res(doc)); //func enqueue
        if (!this.#printing) {
          this.#printDocs();
        }
      }
...
class PrinterNetwork extends Queue {
  #printers = [new Printer(), new Printer(), new Printer()]; //printer network
